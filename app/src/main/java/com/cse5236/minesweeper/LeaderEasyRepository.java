...
public class LeaderEasyRepository {
//    public String TAG = "PlayerRepository";

    private static LeaderEasyRepository instance;
    private ArrayList<Player> dataSet = new ArrayList<>();
    private DAOEasy daoEasy = new DAOEasy();

    /* Singleton Pattern */
    public static LeaderEasyRepository getInstance() {
        if(instance == null) {
            instance = new LeaderEasyRepository();
        }
        return instance;
    }
    ...
    public void setPlayers(MutableLiveData<ArrayList<Player>> data) {
        daoEasy.getAll().addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
